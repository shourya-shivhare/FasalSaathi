...
async def summary_node(state: FasalSaathiState, config: RunnableConfig) -> dict:
    ...
    # ── Step 1b: Inject preferred language into structured data ───────────
    profile = state.get("farmer_profile", {})
    structured["preferred_language"] = profile.get("preferred_language", "ENGLISH")
...
def _build_structured_input(state: FasalSaathiState) -> dict:
    ...
    # Check for stale crop data (>180 days since last update)
    profile = state.get("farmer_profile", {})
    stale_warnings = []
    for crop in profile.get("active_crops", []):
        last_updated = crop.get("last_updated_at")
        if last_updated:
            try:
                from datetime import datetime as _dt
                delta = (_dt.now() - _dt.fromisoformat(str(last_updated))).days
                if delta > 180:
                    stale_warnings.append(
                        f"{crop.get('crop_name', 'Unknown')} (last updated {delta} days ago)"
                    )
            except Exception:
                pass
    ...
    return {
        "sections": sections,
        "errors": state.get("errors", []),
        "stale_crop_warnings": stale_warnings,
    }
...
def _build_presentation_prompt(structured: dict) -> str:
    ...
    lang = structured.get("preferred_language", "ENGLISH")
    lang_directive = (
    ...
    stale_warnings = structured.get("stale_crop_warnings", [])
    stale_note = ""
    if stale_warnings:
        stale_note = (
            "\n\nSTALE DATA WARNING — include this notice in the response:\n"
            "The following crop records have not been updated in over 180 days: "
            + ", ".join(stale_warnings)
            + ". Please update your crop information for more accurate recommendations."
        )
...
9. Structure with clear headers and bullet points
