...
CLI Usage (from project root):
    python ai_service/infer.py --image path/to/image.jpg
    python ai_service/infer.py --image path/to/image.jpg --conf 0.40 --save-dir outputs/
"""
...
_AI_SERVICE_DIR = Path(__file__).resolve().parent      # FasalSaathi/ai_service/
...
def load_model(weights_path: Path | str = DEFAULT_WEIGHTS):
    """
    Load a YOLO model from the given weights file. Caches the model globally
    so it is not reloaded on every inference request.
    ...
    if not weights_path.exists():
        raise FileNotFoundError(
            ...
            "Place your trained best.pt at:  ai_service/models/best.pt\n"
            "Or run training first:          python ai_service/train.py"
        )

    global _GLOBAL_MODEL, _GLOBAL_MODEL_PATH
    if _GLOBAL_MODEL is not None and _GLOBAL_MODEL_PATH == weights_path:
        return _GLOBAL_MODEL

    model = YOLO(str(weights_path))
    _GLOBAL_MODEL = model
    _GLOBAL_MODEL_PATH = weights_path
    return model
